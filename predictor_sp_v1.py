"""
巡夢事件預測系統 - 手機優化版（含內建數據）
簡化操作：點擊即記錄，啟動時自動載入歷史數據
"""
...
# ============ 內建數據（從 Excel 轉換） ============
# 格式：事件類型|子類型（子類型為空時只寫事件類型）
BUILTIN_DATA = [
    "綠", "紫|事件", "灰|失敗", "藍|氣運", "金|事件",
    "綠", "綠", "藍|挑戰卷", "綠", "紫|出遊",
    "綠", "綠", "藍|情報", "金|貼紙", "綠",
    "灰", "綠", "紫|俸祿", "藍", "綠",
    "綠", "藍|氣運", "紫|出遊", "綠", "灰",
    ...
    "藍|健康", "綠", "紫|裝備", "綠", "綠",
    "藍|情報", "金|祭祀", "綠", "灰", "綠",
    "紫|事件", "藍|情報", "紫|活動", "綠", "藍|健康",
    "紫|蘭台", "綠", "灰", "灰", "紫|夢境",
    "綠", "金|貼紙", "綠", "綠", "藍|氣運",
    "綠", "紫|事件", "綠", "綠", "綠",
    "紫|寶石", "灰", "綠", "紫|俸祿", "綠",
    "藍|氣運", "藍|健康", "金|貼紙", "綠", "紫|裝備",
    "綠", "綠", "藍|氣運", "綠", "紫|事件",
    "灰", "藍", "金|祭祀", "綠", "綠",
    "藍|健康", "綠", "紫|靈台", "藍", "綠",
    "紫|寶石", "灰", "綠", "金|鈴鐺", "綠",
...
class Database:
    ...
    def load_builtin_data(self, account_id):
        """載入內建數據"""
        count = 0
        for item in BUILTIN_DATA:
            if '|' in item:
                event_type, subtype = item.split('|')
            else:
                event_type = item
                subtype = ''
            self.add_event(account_id, event_type, subtype)
            count += 1
        return count
...
def main():
    ...
    st.markdown('<div class="sub-header">📱 手機優化 · 點擊即記錄 · 自動學習 · 內建 500+ 筆數據</div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.button("建立帳號", use_container_width=True):
            if new_account:
                ...
                if result:
                    ...
                    # 自動載入內建數據
                    count = st.session_state.db.load_builtin_data(result)
                    st.success(f"✅ 已自動載入 {count} 筆歷史數據")
                    st.rerun()
                else:
                    st.error("❌ 帳號已存在")
            else:
        ...
        if account_id:
            ...
            events = st.session_state.db.get_all_events_ordered(account_id)
            if len(events) == 0:
                if st.button("📥 載入內建數據", use_container_width=True):
                    count = st.session_state.db.load_builtin_data(account_id)
                    st.success(f"✅ 已載入 {count} 筆歷史數據")
                    st.rerun()
